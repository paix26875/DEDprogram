...
if __name__ == '__main__':
    ...
    # キャリブレーションをまとめる変数の定義
    ynew = np.arange(int(min(y)),int(max(y))+1,5)# RGB値（横軸）
    xnew = np.zeros(13)# RGBに対応する温度（縦軸）
    nnew = np.zeros(13)# サンプル数（第2縦軸）
    
    # RGBに対応する温度をynewに格納していく
    # その時のサンプル数をnnewに格納していく
    for temp in range (int(min(y)),int(max(y))+1,5):
        sum=0
        n=0
        # for i in range ((height-1)*(width-1)):
        for i in range (height+width-1):
            if y[i] !=0 and x[i] > RGBmin:
                if y[i]<temp+25 and y[i]>temp-25:
                    # ic(x[i]) 
                    sum += x[i]
                    n += 1
        if n > 10:
    ...
    result_rgb = xnew[xnew.nonzero()]
    result_temperature = ynew[xnew.nonzero()]
    result_samplenumber = nnew[xnew.nonzero()]
